...
ls -la "$TOKENIZER_PATH" 2>/dev/null && echo "   📄 File found!" || echo "   ❌ File not found!"
...
robust_train() {
    ...
    while [ $retry_count -lt $max_retries ]; do
        ...
        if timeout 8h modern-gpt-train --config "$CONFIG_PATH"; then
...
if [ -n "$TOKENIZER_PATH" ] && [ -f "$TOKENIZER_PATH" ]; then
    ...
    train-tokenizer --config "$CONFIG_PATH"
...
echo "🔮 Testing with sample prompt: 'What is an elephant?'"
modern-gpt-infer --config "$CONFIG_PATH" --prompt "What is an elephant?"
